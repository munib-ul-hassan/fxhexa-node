...
const getrealTimeData = async (req, res) => {
    try {
        ...
        const url = `https://live-rates.com/api/price?key=${process.env.key}&rate=${ticket.replace("#","%23")}`
...
const getData = async (req, res) => {
    try {
        ...
        const forexurl = `https://live-rates.com/api/price?key=${process.env.key}&rate=${forex.map((item)=>{return item.split(':')[1].split(",")[1]}).join(",")}`
